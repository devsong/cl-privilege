...
public class RoleServiceImpl implements IRoleService {
    ...
    public Integer createRole(Role role, User user) {
        this.setPersonInsert(role, user);
        return roleMapper.insertSelective(role);
    }

    @Override
    public Integer updateRoleById(Role role, User user) {
        this.setPersonUpdate(role, user);
    ...
    public String getRoleDataTables(RoleSearchModel searchModel) {
        Integer total = this.getRoleTotalBySearch(searchModel);
        List<Role> roleList = this.getRoleListBySearch(searchModel);
    ...
    private void addDataRow(StringBuilder sb, Role r) {
        sb.append("[");
        sb.append("\"<input type=\\\"checkbox\\\" name=\\\"id[]\\\" value=\\\"").append(r.getId())
                .append("\\\">\"");
        sb.append(",").append(r.getId());
        sb.append(",\"").append(r.getName()).append("\"");
        sb.append(",\"").append(r.getRemark()).append("\"");
        sb.append(",\"").append(r.getUpdatePerson()).append("\"");
        sb.append(",\"").append(StringUtil.formatDate(r.getUpdateDate(), "yyyy-MM-dd HH:mm:ss"))
                .append("\"");
        sb.append(",\"").append("<a href=\\\"javascript:Role.update_click('").append(r.getId())
                .append("');\\\" class=\\\"btn btn-xs default btn-editable\\\"><i class=\\\"fa fa-edit\\\"></i> 修改</a>")
                .append("&nbsp;&nbsp;<a href=\\\"javascript:Role.remove('").append(r.getId())
                .append("');\\\" class=\\\"btn btn-xs default btn-editable\\\"><i class=\\\"fa fa-times\\\"></i> 删除</a>")
                .append("&nbsp;&nbsp;<a href=\\\"javascript:Role.assign_click('").append(r.getId())
    ...
    public String getRoleForOptions(Integer userId) {
        List<Role> assignRoles = this.getRoleListByUserId(userId);
        List<Role> allRoles = this.getRoleList();

        Map<Integer, Role> hmAssignRoles = new HashMap<>();
