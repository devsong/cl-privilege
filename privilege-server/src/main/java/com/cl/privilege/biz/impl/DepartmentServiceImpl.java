...
public class DepartmentServiceImpl implements IDepartmentService {
    ...
    public Integer createDepartment(Department department, User user) {
        ...
        List<Department> departments =
                departmentMapper.getDepartmentListByParentId(department.getParentId());
        if (departments == null || departments.size() == 0) {
            ...
            for (Department r : departments) {
                ...
                if (structures.length == parentLevel + 1) {
                    if (StringUtil.isNumber(structures[structures.length - 1]) && StringUtil
                            .compareTo(structures[structures.length - 1], structure) > 0) {
        ...
        this.setDepartmentInsert(department, user);
        return departmentMapper.insertSelective(department);
    }

    @Override
    public Integer updateDepartmentById(Department department, User user) {
        this.setDepartmentUpdate(department, user);
    ...
    public String getDepartmentTree() {
        ...
        if (departmentList == null || departmentList.size() == 0) {
            return ConstantUtil.EmptyJsonObject;
        }
        ...
        for (Department d : departmentList) {
            ...
            i++;
            sb.append("{").append("\"id\":\"").append(d.getId()).append("\"")
                    .append(",\"parent\":\"").append(d.getParentId() == 0 ? "#" : d.getParentId())
                    .append("\"").append(",\"text\":\"").append(d.getName()).append("\"")
    ...
    public List<Department> getDepartmentListForOption() {
        List<Department> departments = this.getDepartmentList();
        if (departments == null || departments.size() == 0) {
            return null;
        }

        List<Department> tempImmediateDepartments = new ArrayList<>();
        ...
        return this.buildDepartmentListForOption(departments, tempImmediateDepartments, "s");
    }

    /**
     * 这种写法和直接对所有部门列表进行循环的写法比较 优点是：更少的循环次数，所以也就是更少的CPU计算和更快的返回时间；缺点是：更多的内存占用 也可以看做是一种空间换时间
     *
     ...
     * @return
     */
    private List<Department> buildDepartmentListForOption(List<Department> descendantDepartments,
            List<Department> immediateDepartments, String structure) {
        ...
        for (Department department : immediateDepartments) {
            ...
            List<Department> tempDescendantDepartment = new ArrayList<>();
            List<Department> tempImmediateDepartment = new ArrayList<>();
            ...
            if (tempDescendantDepartment != null && tempDescendantDepartment.size() > 0
                ...
                List<Department> sub = this.buildDepartmentListForOption(tempDescendantDepartment,
                        tempImmediateDepartment, department.getStructure());
