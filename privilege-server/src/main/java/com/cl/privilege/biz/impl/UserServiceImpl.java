...
public class UserServiceImpl implements IUserService {
    ...
    public List<User> getUserListBySearch(UserSearchModel searchModel) {
        return userMapper.getUserListBySearch(searchModel,
                new RowBounds((searchModel.getPageNo() - 1) * searchModel.getPageSize(),
                        searchModel.getPageSize()));
    }

    @Override
    public Integer createUser(User user, User operator) {
        this.setPersonInsert(user, operator);
    ...
    public Integer updateUserById(User user, User operator) {
        this.setPersonUpdate(user, operator);
    ...
    public String getUserDataTables(UserSearchModel searchModel) {
        Integer total = this.getUserTotalBySearch(searchModel);
        List<User> userList = this.getUserListBySearch(searchModel);
        if (userList == null || userList.size() == 0) {
            return "{\"iTotalRecords\":0,\"iTotalDisplayRecords\":0,\"aaData\":[]}";
        }

        StringBuilder sb = new StringBuilder();
        sb.append(String.format("{\"iTotalRecords\":%d,\"iTotalDisplayRecords\":%d,\"aaData\":[",
                total, total));
        int i = 0;
        for (User u : userList) {
            if (i != 0) {
    ...
    private void addDataRow(StringBuilder sb, User u) {
        sb.append("[");
        sb.append("\"<input type=\\\"checkbox\\\" name=\\\"id[]\\\" value=\\\"").append(u.getId())
                .append("\\\">\"");
        ...
        sb.append(",\"").append(u.getDepartmentName() == null ? "" : u.getDepartmentName())
                .append("\"");
        sb.append(",\"").append(u.getUpdatePerson()).append("\"");
        sb.append(",\"").append(StringUtil.formatDate(u.getUpdateDate(), "yyyy-MM-dd HH:mm:ss"))
                .append("\"");
        sb.append(",\"").append("<a href=\\\"javascript:User.update_click('").append(u.getId())
                ...
                .append(u.getIsLock() ? "User.unlock('" : "User.lock('").append(u.getId())
                .append("');\\\" class=\\\"btn btn-xs default btn-editable\\\"><i class=\\\"fa fa-")
                .append(u.getIsLock() ? "un" : "").append("lock\\\"></i> ")
                .append(u.getIsLock() ? "解锁" : "锁定").append("</a>")
                ...
                .append("');\\\" class=\\\"btn btn-xs default btn-editable\\\"><i class=\\\"fa fa-key\\\"></i> 分配角色</a>")
                .append("\"");
        sb.append("]");
    }
}
