...
public class PrivilegeBaseApiServiceImpl implements IPrivilegeBaseApiService {
    ...
    public String getModuleTree(Integer userId, String visitedModule, String visitedResource) {
        ...
        if (user.getIsAdmin()) {
            ...
            List<String> moduleFlags = new ArrayList<>();
        ...
        String[] icons =
                "fa-gift、fa-cogs、fa-puzzle-piece、fa-sitemap、fa-table、fa-briefcase、fa-comments、fa-group、fa-globe、fa-th"
                        .split("、");
        int iconsFlag = 0;
        ...
        for (Module m : moduleList) {
            ...
            sb.append("</a>");
            sb.append(this.buildResourceTree(m, resourceList, "s", visitedModule, visitedResource));
            sb.append("</li>");
        }
     ...
     * @return
     */
    private String buildResourceTree(Module m, List<Resource> resourceList, String structure,
            String visitedModule, String visitedResource) {
        if (resourceList == null || resourceList.size() == 0) {
            return "";
        }
        int parentLength = structure.split("-").length;
        // 计算一级子资源
        List<Resource> sonList = new ArrayList<>();
        ...
        for (Resource r : sonList) {
            // 递归，找子级的树形结构
            String s = this.buildResourceTree(m, resourceList, r.getStructure(), visitedModule,
                    visitedResource);
            if (visitedResource.contains(r.getStructure())
                    && r.getModuleFlag().equals(visitedModule)) {
                sb.append("<li class=\"active\">");
            } else {
                sb.append("<li>");
            }
            if (s.equals("")) {
                sb.append("<a href=\"").append(m.getUrl()).append(r.getUrl())
                        .append("?visitedModule=").append(r.getModuleFlag())
                        .append("&visitedResource=").append(r.getStructure()).append("\">");
